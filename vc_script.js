...
// Top 100 VC Firms ranked by AUM
...
function renderCard(firm) {
  ...
  const companyPageUrl = `company.html?firm=${encodeURIComponent(firm.name)}&type=vc`;
  ...
  card.innerHTML = `
    ...
    <div class="card__type">VC Firm · ${firm.aum} AUM · ${firm.location || "N/A"}</div>
    ${jobCountHtml}
...
function applyFilters() {
  ...
  document.getElementById("resultsCount").textContent = `Showing ${filtered.length} of ${FIRMS.length} firms`;
}
 
async function init() {
  const res = await fetch("vc_firms_data.json");
  FIRMS = await res.json();
  document.getElementById("totalCount").textContent = FIRMS.length;
 
  // Load intern status
  try {
    const statusRes = await fetch("vc_intern_status.json");
    if (statusRes.ok) {
    ...
    // vc_intern_status.json not available yet
  }
